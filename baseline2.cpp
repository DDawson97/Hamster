...
int main() {
    ...
    // (Do this while the graph is still perfectly snapped to the 0-6 grid)
    // ---------------------------------------------------------
    json out;
    for (node v : G.nodes) {
        // Note: It's safer to use std::stoi(GA.label(v)) to get your original ID back 
        // rather than v->index(), just in case OGDF ordered them differently!
        out["nodes"].push_back({
