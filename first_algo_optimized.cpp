...
int main(int argc, char* argv[]) {
    ...
    // ==========================================
    // PHASE 3: LOCAL OPTIMIZATION (Delta Update)
    // ==========================================
    fastEvaluateCrossings(G, GA, all_edges, edge_idx, current_crossings, current_k, current_total);
    ...
    for (int step = 0; step < 40000; ++step) {
        ...
        // --- OPTIMIZATION 2: THE DELTA UPDATE ---
        std::vector<int> new_crossings = current_crossings; // Fast copy
        int new_total = current_total;
        
        // Step A: Subtract old intersections involving V
        for (adjEntry adj : v->adjEntries) {
            ...
            for (edge e_other : all_edges) {
                ...
                if (doIntersect(old_x, old_y, GA.x(e_v->opposite(v)), GA.y(e_v->opposite(v)),
                    ...
                    new_crossings[ev_idx]--;
                    new_crossings[edge_idx[e_other]]--;
                    new_total--;
                }
            }
        }

        // Step B: Temporarily apply the move
        GA.x(v) = new_x; GA.y(v) = new_y;

        // Step C: Add new intersections involving V
        for (adjEntry adj : v->adjEntries) {
            edge e_v = adj->theEdge();
            int ev_idx = edge_idx[e_v];
            for (edge e_other : all_edges) {
        ...
        if (new_k < current_k || (new_k == current_k && new_total < current_total)) {
            ...
            current_crossings = new_crossings; // Commit the crossing changes
            if (step % 100 == 0 || current_k == 0) std::cout << "  -> Step " << step << " | Improvement! k: " << current_k << " | Total: " << current_total << "\n";
        } else {
